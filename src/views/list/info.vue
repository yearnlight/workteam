      ...
      <el-tab-pane label="任务详情" name="detail">
        <el-form ref="elForm" :model="formData" size="small" label-width="160px" label-suffix="：">
          ...
          <el-form-item label="预估日期范围" prop="startEndTime">
            <span :class="{'line-through':formData.status == 'shelve' || formData.status == 'waitAssign'}">{{formData.startTime}} - {{formData.endTime}}</span>
          </el-form-item>
          <el-form-item label="预估时间" prop="estimatedTime">
            <span :class="{'line-through':formData.status == 'shelve' || formData.status == 'waitAssign'}">{{formData.estimatedTime | formatDur}}</span>
          </el-form-item>
        ...
        <el-timeline>
          <el-timeline-item :color="$util.displayEnum($enum.statusList,item.status).color" v-for="(item,index) in logs" :key="index" :timestamp="item.updatetime" placement="top">
            <el-card>
              ...
              <p>
                <label>状态：</label>
                <span :style="`padding: 2px 6px !important;color: #fff !important;background-color: ${$util.displayEnum($enum.statusList,item.status).color};`">{{$util.displayEnum($enum.statusList,item.status).label}}</span>
              </p>
              <p>
                <label>进度：</label>
                {{item.finished}}%
