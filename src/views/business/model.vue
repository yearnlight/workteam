<template>
  <div class="demo">
    ...
    <el-dialog title="生成代码" :visible.sync="isGenerate">
      <el-form :model="projectForm" ref="projectForm" label-suffix="：" label-width="120px">
        <el-form-item label="项目名称" prop="project" :rules="[{ required: true, message: '项目名称不能为空'}]">
          <el-input v-model="projectForm.project" autocomplete="off"></el-input>
        ...
        <el-form-item label="服务Code">
          <span>{{projectForm.code}}</span>
        </el-form-item>

      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="isGenerate = false">取 消</el-button>
        <el-button type="primary" @click="saveGenerate('projectForm')">确 定</el-button>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    generate() {
      ...
      that.projectForm.project = that.config.project;
      that.projectForm.code = that.config.code;
    },
    saveGenerate(formName) {
      let that = this;
      this.$refs[formName].validate((valid) => {
        if (!valid) {
          return false;
        }
        this.$axios.post("/server-config/generate", { uuid: that.config.uuid, project: that.projectForm.project }).then(res => {
          if (res.status == 200) {
